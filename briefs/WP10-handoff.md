...
## A note on the shared recordings folder

`node scripts/check.mjs` with no arguments scans the whole shared `../tessera-recordings/` folder, which by design (CLAUDE.md rule 8) also holds other worktrees'/sessions' concurrent recordings — while working on this WP, sessions I did not create (e.g. `cache-*` ones from what looks like a batch `npm run cache` run, some timestamped before this session even started) appeared there too. A repo-wide `npm run check` will report on those as well; they are not WP10 output. `briefs/WP10-report.md` §1 lists exactly the four sessions (plus the two re-pinned exemplars) this WP is actually responsible for — check those explicitly (as its own commands do) rather than trusting the repo-wide count.
