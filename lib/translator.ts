 ...
 *
 * WP7 (paired with style sheet v0.6, lib/prompt.ts): rule 9's old clause
 * (a) required every beat to say how the previous beat's headline chip
 * leaves before the new one lands. That fought image-to-video chaining —
 * WP0 found chaining inherits the previous composition, which is what
 * makes the cut seamless and is also what covers a headline sometimes
 * (WP0-report.md §7) — so WP7 drops the clause (headline occlusion is
 * accepted, not fought) and the exemplar's chip-exit language below. No
 * schema change, so this did not need its own version bump; it landed in
 * the same v0.3.2 as the connector/tag fields above.
 */
 ...
 *
 * v0.3.2 (WP7, same version): beats 2-6's `action` no longer states how the
 * previous beat's headline chip leaves (rule 9's dropped clause) — each now
 * opens straight on the shape-match transformation.
 */
export const EXEMPLAR_NDJSON = [
  ...
  `{"scene":1,"line":"Six months earlier it was three point one one million.","headline":"$3.11M → $1.85M","ground":"violet","subjects":["the coin stack","a torn-paper arrow"],"action":"The coin stack shrinks coin by coin; a torn-paper arrow enters and points down at it.","hand":false,"handoff":"the arrow","hero":false,"scale":"small","delivery":"[fast-paced] Six months earlier it was three point one one million.","source":[9,10],"connector":{"kind":"paper ribbon","from":"the coin stack","to":"a torn-paper arrow","colour":"black"},"tag":null}`,
...
export function translatorSystem(clipSeconds: 5 | 10 | 15 = 5): string {
  ...
  const actionRule =
    clipSeconds === 10
      ? `9. action. This shot is ${clipSeconds}s long and may carry one internal shape-match cut at the midpoint: write "action" as two timecoded halves, "[0-5s] ... [5-10s] ...". The first half is one clear cause-and-effect movement with a start and a landing, exactly as in a 5s beat (including, on a beat after the first, how the previous handoff shape transforms into this beat's opening subjects — a shape-match cut, not a description of the previous headline chip leaving; a new headline may land over or beside what is already there). The second half either continues that same movement to a further landing (no internal cut; still write both timecoded halves) or cuts once, mid-shot, to a second composition that develops the same subject further. Either way the shot ends holding on one named "handoff" shape. The whole programme cuts on matching handoff shapes between shots, with a clean headline change each time. The final beat is the bookend: its action returns to scene 1's primary subject, and its "handoff" is the exact string scene 1's first beat used for its own "handoff", with one element changed or added since scene 1.`
