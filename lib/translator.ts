 ...
 * (b) required every beat to say how the previous beat's headline chip
 ...
 * accepted, not fought) and the exemplar's chip-exit language below. The
 * Beat schema itself is unchanged; TRANSLATOR_VERSION stays translator-v0.3.1.
 */
...
9. action. One clear cause-and-effect movement with a start and a landing: what enters, what it does, where it holds. The first beat opens cold. Every later beat states how the previous handoff shape itself transforms into this beat's subjects — a shape-match cut: the closing shape of the previous composition becomes the opening shape of this one. Do not describe the previous headline chip leaving, sliding off, or clearing first; a new headline may land over or beside what is already there. The final beat is the bookend: its action returns to scene 1's primary subject, and its "handoff" is the exact string scene 1's first beat used for its own "handoff", with one element changed or added since scene 1.
