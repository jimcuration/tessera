...
> **Note (WP5 merge):** a WP5 live render put recognisable human faces on paper "tags" whose text description was fully rule-6-compliant (`briefs/WP5-report.md` §5) — a real CLAUDE.md rule 6 violation from the video model itself, not the translator. WP5 mitigated it by extending lines 3-4 to rule out faces/photographs on any tag or card, verified clean on one re-render. That wording is **not** in the lines above — restoring v0.3 exactly, as asked, means this specific mitigation is not currently live. Flagging so this doesn't read as solved; see the WP5 docs before the next live-render pass.
