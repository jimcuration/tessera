...
> **Note (WP5 merge):** a WP5 live render put recognisable human faces on paper "tags" whose text description was fully rule-6-compliant (`briefs/WP5-report.md` §5) — a real CLAUDE.md rule 6 violation from the video model itself, not the translator. WP5's mitigation extended both lines 3 and 4 to rule out faces/photographs on any tag or card, verified clean on one re-render. Restoring v0.3 to main dropped both; line 3's sentence (above) has since been folded back in — it stays until a proper code-side face gate lands, not because line 3 alone is considered sufficient. Line 4's parallel wording (tags/cards as blank rectangles) is still not present. See the WP5 docs for the full finding before the next live-render pass.
